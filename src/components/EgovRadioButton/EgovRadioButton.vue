<template>
    <div class="egov-radio-group">
            ...
            >
                ...
                <template #default="{ isSelected, isDisabled }">
                    <span style="display: none;">
                        {{ console.log('isSelected:', isSelected, 'value:', option.value, 'internalValue:', internalValue) }}
                    </span>
                    <i
                        :class="[
                            internalValue === option.value ? 'ri-radio-button-line' : 'ri-checkbox-blank-circle-line',
                            {
                                'text-disabled': isDisabled,
                                'text-primary': internalValue === option.value && !isDisabled
                            }
                        ]"
                    ></i>
                </template>
                ...
                <template #radio="{ isSelected, isDisabled }">
                    ...
                    <i
                        :class="[
                            internalValue === option.value ? 'ri-checkbox-circle-fill' : 'ri-checkbox-blank-circle-line',
                            {
                                'text-disabled': isDisabled,
                                'text-primary': internalValue === option.value && !isDisabled
                            }
        ...
        <!-- Debug output to verify internalValue -->
        <div style="font-size: 12px; color: #666;">Debug: Selected Value = {{ internalValue }}</div>
    </div>
</template>
...
watch(internalValue, (newVal) => {
    console.log('internalValue updated:', newVal)
...
const handleRadioClick = (value) => {
    if (!props.disabled && !props.options.find(opt => opt.value === value).disabled) {
        console.log('Radio clicked, setting internalValue to:', value)
        internalValue.value = value
    }
}

// Handle v-radio-group modelValue update
const handleUpdate = (value) => {
    console.log('v-radio-group updated:', value)
    internalValue.value = value
}
</script>
